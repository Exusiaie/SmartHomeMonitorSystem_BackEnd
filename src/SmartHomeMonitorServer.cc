...
// 初始化数据库表
    ...
    MySQLClient db;
    if(db.connect(
        config->getConfigMap()["db_host"],
        std::stoi(config->getConfigMap()["db_port"]),
        config->getConfigMap()["db_user"],
        config->getConfigMap()["db_passwd"],
        config->getConfigMap()["db_database"])) {
        
        if(!UserTableCreator::tableExists(db)) {
            UserTableCreator::createTable(db);
        }
    }
}
    ...
    case wd::TASK_TYPE_LOGIN_SECTION2:   //如果是第二阶段
     ...
     case wd::TASK_TYPE_REGISTER_SECTION1: {
        ...
        case wd::TASK_TYPE_REGISTER_SECTION2: {
            UserRegisterSection2 userReg2(conn, packet);
